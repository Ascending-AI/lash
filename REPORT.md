...
- **Around sixty `include!`d files elsewhere in the repository are still
  unformatted and unchecked.** The scan covers `crates/lashlang`; widening it is
  a separate cleanup, and adding a scope is a one-line change once a crate is
  clean.
...
| `heap_deep_chain_mutation_32` | 3.01 ms | 2.86 ms | 1.05x faster |
...
| --- | --- | --- | --- |
| `large_data` allocated bytes | 2,400,000 | unchanged | Measures 2,264,799. |
| `type_system_stress` allocated bytes | 2,200,000 default | 2,400,000 | The alias-copy cost of deep isolation on large record literals. Measures 2,305,763, 4.1% headroom. |
| Aggregate profile instructions | 12,000 | 110,000 | The aggregate now includes nine heap scenarios that deliberately execute long loops. Measures 72,641. |
| RLM short-binding commit ceiling | 34 KiB | 48 KiB | Investigated: the commit did not grow. It is 33,027 bytes and has been since the tree representation. The tight assert was replaced because it failed on harmless payload changes while the assertion that carries the meaning — no leaf minted — is separate. |
| New heap scenarios | — | ns, allocation and byte budgets from measurement | Nine scenarios; wall-clock at roughly 3x measured, counts at roughly 20%. |
| Chain depth ratio | — | 5.0 | New. Measures 2.3x. |
...
continuation, the deep-chain decode, the two rejected-patch regressions, and the
