...
[way-of-working.md](way-of-working.md) says *where* each artifact goes. This says *how a ticket reads* once you're writing one. It applies to every FIG issue (bugs, features, wayfinder tickets, PR slices), drafted by a human or an agent.
...
- A big or foggy effort → a **wayfinder map** with child tickets ([way-of-working.md](way-of-working.md)).
...
"endpoint returns paginated results", "ADR-N is recorded and linked here".
...
- **Bug.** Write `## Why` as a narrated incident, not an abstract symptom: *"User uploaded a CSV → the assistant answered unrelated questions → the document never appeared in the panel."* Concrete beats "document handling is unreliable."
...
This is how wayfinder already resolves a `wayfinder:research` or `wayfinder:grilling` ticket: the body carries the question and names the human who can decide it, `save_comment` records the finding or decision, and the map's index gets a one-line gist plus a link. A research ticket that keeps its body high-level and its report in a comment stays scannable *and* carries the full depth; the two aren't in tension.
...
- **Never cite a non-durable artifact as the sole record.** A session scratchpad path, an unmerged branch, or a Slack message doesn't count as linked evidence; they rot or vanish. Only a resolved comment on the ticket, a PR pinned to a commit SHA (not a branch), an ADR, or a repo file is a durable link. (This overrides the `/research` skill's "write findings to a repo file" default and wayfinder's "throwaway `research/<name>` branch" mechanic; in this repo the durable home for a research finding is a comment on the ticket.)

## Titles

A plain sentence naming the outcome. `Chat thread list is slow and unbounded, needs pagination` reads at a glance. A 20-word RCA clause like `non-Restate AwaitEvent is process-local; SQLite hosts advertise Durable for a sticky path` is a line out of the investigation, not a title. The key rides inside the linked title; never refer to a ticket by bare `FIG-123`.

## Every non-trivial ticket has an orienting parent

Anything past a single-PR slice hangs off an orienting parent: a `wayfinder:map` for foggy efforts, or a plain parent issue for a known multi-PR arc. This is the "overarching ticket" that lets a reader orient before diving in.
