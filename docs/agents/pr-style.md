...
- **Public facade changes.** A diff that moves, adds, or re-homes a `pub use` in `crates/lash/src/lib.rs` is not covered by `cargo check --workspace` or by `just battery-fast`, both of which compile the facade from inside the workspace. Run `just seal` — the same `--test ui` invocation as CI's `seal` lane — so a re-export move fails locally instead of on CI.
