...
`just push-gate` and the confidence lanes are opt-in full diagnostics for unusual-risk changes, release work, or an explicit user request; they are not routine push, review, or merge prerequisites. When run, heavy gates are serial within a lane and capped across the box: the Kiln fork's `env.sh` sets `CARGO_BUILD_JOBS=8` and `NEXTEST_TEST_THREADS=4`, and the build-heavy legs of `push-gate.sh` run through the `heavy-slot` semaphore, so concurrent lanes queue instead of thrashing. These controls are inert on CI. Do not override either without a measured need; the mechanics are in `CONTRIBUTING.md` under "Concurrent local gates".
