...
ADRs (`docs/adr/`), the agent runbooks you are reading (`docs/agents/`), and the procedure
document `docs/store-sql-authoring.md` (FIG-3380). There are no
...
1. **Read the trace or structured log for the operation in question** before designing any wait-and-observe or perturbation experiment (the `lash-trace` crate docs cover what Lash emits and how to export it). Most questions of the form "what did it read and why did it decide that" must be answerable from the recorded evidence.
...
`just push-gate` and the confidence lanes are opt-in full diagnostics for unusual-risk changes, release work, or an explicit user request; they are not routine push, review, or merge prerequisites. When run, heavy gates are serial within a lane and capped across the box: the Kiln fork's `env.sh` sets `CARGO_BUILD_JOBS=8` and `NEXTEST_TEST_THREADS=4`, and the Kiln `cargo` wrapper runs every compile-shaped command in its own cgroup scope inside the fixed `kiln-heavy.slice` aggregate, so concurrent lanes are capped instead of thrashing and none of them queues. These controls are inert on CI. Do not override either without a measured need; the mechanics are in `CONTRIBUTING.md` under "Concurrent local gates".

When an opt-in battery is warranted, it may consult `python3 scripts/ci_plan.py gate-scope --base origin/main` to skip gate families no touched path can reach — a prose-only change does not need the compile battery. `scripts/ci_plan.py` is the repository's one change classifier: CI's job plan, this gate scope and `scripts/dev-test.py` all project the same path table. It only ever skips what it can prove is unaffected: a shared input (root manifest and lockfile, toolchain, `.cargo/`, `.config/`, `scripts/`, `.github/`), an unrecognised path, an empty path set, or its own failure runs everything. Its decision line must be printed verbatim into the gate log next to the gate table, so a reviewer can audit every skip rather than take it on trust; a battery result reported without that line is reported as if nothing was skipped.

### Iterating: change-scoped fast tests

`python3 scripts/dev-test.py` is the implementer-loop default, run before calling a change done; while editing, prefer `kiln test <label> --test_arg=<name>` on the binary that owns the behavior. The script runs `//:dev_tests` — `//:workspace_tests` minus the dev-deferred binaries — narrowed to changed packages using the generated developer inventory. Complete batches run once; their individual members, deferred tests and manual service gates are not added by package wildcards. Shared compiler/configuration inputs and unknown paths widen to the Rust suite, with repository gates for shared tooling. Known Python test edits run their exact CI inventory command; mapped validation scripts run their direct proof. A docs-only diff runs nothing. One blind spot is handled by name rather than by the wildcard: the trybuild compile-fail fixtures on the public API surface are the `manual`-tagged `//crates/lash:ui_fixtures` target, so ordinary developer selection and `cargo check --workspace --all-targets`, which treats `tests/ui/` as data, do not compile them. `dev-test.py` names that label explicitly when the diff touches `crates/lash` or a root manifest, the same path set CI classifies as facade-affecting (FIG-2801); CI itself runs it on every trusted event, inside the core Bazel partition. It refuses live Postgres/S3 URLs: those jobs belong to CI and fight over ports (`KILN_GATE_ID`) on this box. CI owns the complete `//:workspace_tests` partition on merge groups, the services, E2E, the deferred Unicode suite, and the workbench browser test. A trusted pull request runs the core partition leg (`//:workspace_tests` minus its tail, plus the API seal), the workspace clippy, and — whenever its diff can move a Rust build — the feature-lane compile and clippy (`//:feature_lanes //:feature_lane_clippy`, because a lane break rides an upstream API change rather than the gated files); `//:feature_lane_tests` joins when the diff touches feature-gated code, and when the diff reaches `lash-postgres-store`'s closure its PG 16 suites run too. The merge group adds the tail leg, the full lane board and, for a schema diff, PG 14/18, all on the combined tree. Deferred Unicode and the lashlang consumer live on the release dispatch.
...
- Trunk-based: `main` is the only long-lived branch. On the shared box, create one `kiln fork lash <name>` per agent, work from its `/merged` checkout after sourcing `env.sh`, and remove it with `kiln rm lash <name>` after merge. Branch from fresh `origin/main`, use short-lived branches, and merge by PR. Never write under `golden-*`, remove a fork with `rm -rf`, push product changes straight to `main`, or tag or publish a release by hand; `CONTRIBUTING.md` carries the full development and release process.
