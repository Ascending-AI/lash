...
- `source_key` is the immutable idempotency key for a submitted revision. Exact replay of the same source key and submitted ingress/input content returns the existing record. Reusing the source key with changed ingress or input content is a store conflict.
  - Amendment (FIG-3544): replay compares only a submission digest written once at admission beside the submitted ingress (the ingress as submitted plus the canonical input JSON, excluding generated identity), never the row's current ingress, which a Defer rewrites; the conflict reaches the host as the typed `DurableIdentityConflict`.
