...
one FIFO prefix, and only session commands stay unbatched, except that adjacent
config patches coalesce. Every wake terminal raises the redelivery floor in its
own transaction, and a wake content conflict is a non-blocking discard (ADR 0101
§8, §9).
