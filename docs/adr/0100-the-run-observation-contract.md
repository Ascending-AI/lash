...
**One process cursor (FIG-3571 phase 2a).** That encoding is
`lashpc2:<epoch>:<process-reference>:<position>:<sequence>`, one token across
durable history and the live hub. `position` is the publication position the
holder has seen; `sequence` is the durable event high-water mark it has seen.
`<process-reference>` is one opaque component naming the exact lifetime: today
the process id and incarnation, and FIG-3607 swaps that single component to
the minted process id. `Processes::events` pages the durable history from the
cursor, with Full or Lite as a request parameter, and returns the next cursor.
Durable commits reach the hub as `Committed { sequence }` items through the
ADR 0017 sink after the commit; publication never fails the commit. A
subscription resumes silently only when the ring holds every item after the
cursor's position and its `Committed` evidence bridges the cursor's sequence
to the durable high-water mark, the session rule applied per durable event.
Otherwise — an epoch change, trim, incarnation change, unknown or pruned
process, or an unbridged sequence, even for an idle process — it answers
`Gap` with a snapshot and a new cursor. The snapshot names one exact lifetime
and durable high-water sequence: status plus the effect-summary fold through
it, read with Full payloads under a bounded acquisition budget, with typed
retention outcomes, and durable-summary completeness reported apart from
live-graph completeness. Under the current, temporary clean-cutover policy a
`lashpc1` cursor or a `process-event-page:v1` token is refused, typed, naming
