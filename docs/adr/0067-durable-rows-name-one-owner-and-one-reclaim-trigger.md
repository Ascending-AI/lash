...
| `effect_scope_retirements` / `lash_effect_scope_retirements` | Scope: the process or runtime operation whose journal was retired as unreachable; written by the scope-exact retirement as its commit point. On PostgreSQL the fence and the journal deletions are one transaction. On SQLite the fence of a registered process lives in the process registry file (the registry's own `effect_scope_retirements` table) and commits first, and the journal purge is a second, idempotent transaction that the next host bind or reclaim sweep repeats if it was lost; runtime-operation fences stay in the journal file with their rows. | Permanent for runtime-operation scopes (used-once ids). A process fence is released only by the same process id being registered again, and the release is the registry insert's own write: the row is deleted in the same single-file transaction as the insert, never by a caller, so a failed insert rolls the release back and a crash leaves the id fenced-and-unregistered or registered-and-unfenced, never neither (ADR 0049); no retention or vacuum sweep removes either. |
| Session-free rows of `runtime_effect_replay` / `runtime_effect_group` / `await_event_waits` (`lash_` on PostgreSQL) | Scope: the runtime operation the facade minted for one plugin command or task (`{session_id}:plugin_command:{name}:{TurnActivityId}` / `{session_id}:plugin_task:{name}:{TurnActivityId}`, minted by `lash_core::store::mint_facade_operation_id`); the operation's receipt (`runtime_turn_commits` keyed by the operation) is the proof it will never run again. | Receipt-at-quiescence: the facade retires the scope `when_quiescent` as the receipt returns. A scope still live at that moment (an in-progress child, an open group, an awaited unresolved promise) is left alone and owned by the host-invoked evidence sweep (`reclaim_retained_evidence`), which under the same fence lock retires every facade-minted operation scope (`is_facade_minted_operation_id`, the mint's inverse) whose receipt is recorded and that is quiescent at sweep time, and never one without a recorded receipt. A caller-supplied operation id is never swept: its receipt is the caller's replay proof and only the caller knows when the id is spent. The sweep also keeps the receipt of a still-live scope past the host's horizon, so the proof outlives the drain it waits for. |
