...
findings, reports the freeze, and exits 0 (FIG-3846). Removing the key
...
def decode_patch(raw: bytes) -> str:
    ...
    # "+++ ", shape/fingerprint rows, and the version constants in the
    # component sources) are always valid UTF-8, so replacement characters in
    # payload lines cannot mask a payload-shape change or a missing bump.
...
def main(argv: list[str]) -> int:
    ...
    return 0 if valid or freeze else 1
