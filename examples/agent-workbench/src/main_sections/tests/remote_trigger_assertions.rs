...
pub(crate) async fn assert_remote_started_process_surface(
    ...
    for process_id in process_ids {
        ...
        let lash::process::ProcessEventReadOutcome::Retained(page) = &outcome else {
            panic!("a started process's history is retained");
        };
        ...
        let lash::process::ProcessEventReadOutcome::Retained(round_trip_page) = &round_trip_outcome
        else {
            panic!("round trip preserves the retained page");
        };
        let lash::process::ProcessEventPageEvents::Full(round_trip_events) =
            &round_trip_page.events
        else {
