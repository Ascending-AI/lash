...
async fn await_work_route_returns_terminal_outcome_and_reconciled_events_inner() {
    ...
    let Json(result) = await_work(
        AxumPath(await_route_proc_id.to_string()),
        State(state.clone()),
    )
    .await
    ...
    let failed = work
        .iter()
        .find(|item| item.process.process_id == failed_work_rail_proc_id)
...
async fn session_delete_reclaims_the_deleted_sessions_terminal_work_inner() {
    ...
    assert!(
        matches!(
            process_registry.get_process(&ids[reclaimed.as_str()]).await,
