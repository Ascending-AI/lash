...
4-vCPU runner. Because it is that cheap it runs on pull requests as well as
merge groups, where the two Cargo matrices it replaces were merge-group-only.
Untrusted (fork) pull requests have no cache credentials and so no pool: they
keep the Cargo commands unchanged. `docs/agents/hermetic-build.md` records how
the resolution is computed and reconciled against Cargo, and the one
faithfulness limitation that remains.
