...
A pull request runs the fast board — lint, the hygiene jobs, and the
path-gated seal and repository-gate jobs — plus a `bazel-tests` leg that runs
only the affected Bazel targets the plan selects: the same selection
`python3 scripts/dev-test.py` computes locally, so the CI run is what your
dev-test dry-run printed. Feature lanes join it only for a diff that touches
feature-gated code. Merge groups keep the broad board: the whole workspace
core suite and tail, the selected store suites, and every trusted Rust job.
The single `CI conclusion` job rejects failed, cancelled, missing, or
...
4-vCPU runner. It runs on every trusted merge group and dispatch, and on a
pull request only when the diff touches feature-gated code; deferred Unicode
and the lashlang consumer remain dispatch-only.
...
profile release.yml certifies against. `Feature lanes` still runs on trusted
merge groups and on pull requests whose diff touches feature-gated code. The
...
The workers E2E family runs on full-profile (`workflow_dispatch`) runs only.
Pull requests no longer run it — the `ci:workers` label opt-in is retired —
and it does not execute in the merge queue. Run a local worker recipe
