...
Tick-time `agent_workbench.cron.restate.zombie_cancelled` is out of scope for this browser runbook: the supported session-delete path proactively cancels the session's cron jobs, so no scheduled tick can observe the retired session. That crash-window defense-in-depth is owned by the FIG-1018 `cron_tick_decision_cancels_a_retired_session_with_typed_trace` and `cron_tick_cancels_a_retired_session_with_typed_decision` tests in `examples/agent-workbench/src/restate/tests/cron_tests.rs`; FIG-1071 extends the decision with registration disposition and adds a live-session backstop that cancels a tick whose registration was deleted or disabled. A pre-FIG-1071 in-flight invocation that journaled only the legacy session disposition replays as live/enabled and may emit or re-arm once before its next tick self-cancels; treat that as a bounded rollout exception, not immediate post-deletion silence.
