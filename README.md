...
A session id reaches Lash through two terminal verbs. `core.session(id).open()`
...
another process. Poll a queue with `durable()`, never with `open()`. An open
session exposes the same handle as `session.durable()`.
