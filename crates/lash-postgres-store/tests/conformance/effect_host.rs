...
// Parked on this tier alone: the law flakes Contended on the Postgres effect
// engine, which FIG-3667 deletes wholesale.
lash_conformance::effect_group_close_race_tests!(
    #[ignore = "flakes Contended on the Postgres effect engine; the engine is deleted by FIG-3667"]
    {
        let Some((database_lock, storage)) = storage().await else {
            eprintln!(
                "skipping Postgres effect-group conformance: LASH_POSTGRES_DATABASE_URL is not set"
