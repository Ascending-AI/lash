...
async fn postgres_refuses_completed_pre_frame_key_continue_as_at_open_when_configured() {
    ...
    assert_eq!(
        message,
        format!(
            "store backend error: Postgres schema component `lash-postgres-store` has version 43, expected {expected}. That database was provisioned by an older build: component 43 predates this build's component {expected} and has no applicable migration. The component schema is normally a reject-and-recreate boundary. This build declares a forward migration into component {expected} only from component 101, 102, 103, 104, 105, 106, so component 43 has no upgrade path. Drain the affected sessions and recreate the whole Lash trust domain with this build: provision the database from the DDL artifact this build ships (`PostgresStorage::schema_ddl()`, committed as crates/lash-postgres-store/schema.sql), then reset the session tombstones, the await-event revocation ledger, the effect journal, and the Restate state together — any one of them left behind still refers to sessions the recreated database does not have. docs/adr/0081-destructive-schema-changes-are-currently-reject-and-recreate.md records why this boundary refuses instead of migrating. This gate is unconditional; SchemaCheck::WarnOnly does not relax it."
        )
    );
}
