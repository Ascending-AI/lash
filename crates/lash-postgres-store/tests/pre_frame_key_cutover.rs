...
async fn postgres_refuses_completed_pre_frame_key_continue_as_at_open_when_configured() {
    ...
    assert_eq!(
        message,
        "store backend error: Postgres schema component `lash-postgres-store` has version 43, expected 87. The component schema is normally a reject-and-recreate boundary. This build has legacy declarations from component-50 through component-61 plus the component-62 forward migration, but every published pre-61 graph shape carries the retired sequence column and is refused before migration DDL can run. Component 61 is a hard append-identity cutover with no applicable migration. This mismatch has no applicable migration. Drain affected sessions and recreate the whole Lash trust domain with this version: provision the database from this build's schema.sql artifact, and reset the tombstones, await-event revocation ledger, effect journal, and Restate state together; see docs/persistence.html#delete-sessions. This gate is unconditional; SchemaCheck::WarnOnly does not relax it."
    );
}
