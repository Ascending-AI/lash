...
async fn report_remedies_match_the_finding_class() {
    ...
    scratch
        .apply(
            "UPDATE lash_schema_versions SET version = 45 WHERE component = 'lash-postgres-store';
             DROP INDEX idx_lash_process_events_key",
        )
        .await;
...
async fn the_schema_gate_emits_its_decision_basis() {
    ...
    assert_evidence(
        ...
        &scratch.name,
        "allowed",
        &["found_version=Some(45)", "finding_total=0"],
    );
...
fn assert_evidence_with_provisioning(
    ...
    for field in ["component=lash-postgres-store", "expected_version=45"]
        .iter()
