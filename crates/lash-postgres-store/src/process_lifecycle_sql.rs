...
//! predicate.
//!
//! Every worklist, prune, preflight and observer statement builds its filter
//! from [`lash_core::ProcessStatus`] and [`lash_core::WakeDeliveryState`]
//! through these helpers, so a new variant cannot compile without declaring
//! which side of the live/retired (or delivered/undelivered) partition it falls
//! on. `process_lifecycle_vocabulary` in `lash-sim` fails if a raw lifecycle
//! literal reappears in a statement here.
