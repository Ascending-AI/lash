...
//! [`require_settleable_turn_input`](lash_core_execution::store_backend_support::require_settleable_turn_input),
//! over the row this module locks `FOR UPDATE` — now reached through the
//! shared [`plan_turn_input_settlement`](lash_core_execution::store::claim_plan::plan_turn_input_settlement)
...
pub(crate) async fn plan_turn_input_settlement_tx(
    ...
    completed: &lash_core_execution::TurnInputCompletion,
) -> Result<lash_core_execution::store::claim_plan::TurnInputSettlementPlan, StoreError> {
    ...
    lash_core_execution::store::claim_plan::plan_turn_input_settlement(completed, rows)
        .into_result()
}
