...
pub(super) const SCHEMA_MIGRATIONS: &[SchemaMigration] = &[
    // Retain component 87 as the refusal-only endpoint. No row targets component
    // 88: recoverable artifact-cleanup evidence is a reject-and-recreate cutover.
    // Component 87 adds exact artifact ownership to main's component-86 admitted
    // effect identity and pending-input claim correlation shape.
    SchemaMigration {
        from: 86,
        to: 87,
        source_missing_tables: &["lash_artifact_owners", "lash_artifact_owner_retirements"],
    ...
    // Component 86 combined incompatible component-85 parents as a destructive
    // cutover, so component 85 remains a refusal-only source at this endpoint.
    SchemaMigration {
        from: 85,
        to: 87,
