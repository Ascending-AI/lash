...
pub(super) const SCHEMA_MIGRATIONS: &[SchemaMigration] = &[
    ...
    // Retain component 89 as the refusal-only endpoint. No row targets component
    // 90: process attachment owners now require a registry-minted incarnation,
    // so old stores are recreated before any bare-id row can be decoded.
    SchemaMigration {
        from: 83,
        to: 89,
        source_missing_tables: &[],
