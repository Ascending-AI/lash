...
pub(super) const SCHEMA_MIGRATIONS: &[SchemaMigration] = &[
    ...
    // Retain component 88 as the refusal-only endpoint. No row targets component
    // 89: exact artifact-owner edges and the durable cleanup ledger have no safe
    // backfill, so old stores are recreated before any row is decoded.
    SchemaMigration {
        from: 83,
        to: 88,
        source_missing_tables: &[],
