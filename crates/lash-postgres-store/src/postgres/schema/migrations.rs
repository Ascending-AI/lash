...
pub(super) const SCHEMA_MIGRATIONS: &[SchemaMigration] = &[
    // Component 88 is the FIG-677 cutover. Component 87 remains a refusal-only
    // source: it must be recreated before exact artifact-owner and
    // cleanup-evidence rows can be read. Historical admissions are retargeted
    // to the current component so `apply_schema_migration` can select them.
    SchemaMigration {
        from: 87,
        to: 88,
