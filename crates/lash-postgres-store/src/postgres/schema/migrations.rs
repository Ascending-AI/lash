...
/// guest asked for (component 96). Component 97 is therefore retained as the
/// refusal-only endpoint and no row targets component 98.
///
...
pub(super) const SCHEMA_MIGRATIONS: &[SchemaMigration] = &[
    ...
    SchemaMigration {
        ...
        // creates the named process-definition registry (FIG-2995) and
        // component 98 the release stamp (FIG-3092). No predecessor records
        // existed at any of these moves, so the retained endpoint carries all
        // four: a pre-cutover store is refused at open rather than migrated
        // (its schema lacks both relations and the retained trigger capture).
