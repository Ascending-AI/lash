...
pub(super) const SCHEMA_MIGRATIONS: &[SchemaMigration] = &[
    // Component 88 is the refusal-only FIG-677 cutover. No row targets it:
    // component-87 stores must be recreated before exact artifact-owner and
    // cleanup-evidence rows can be read. Every historical migration still
    // targets component 87, the lifecycle-policy generation supplied by main.
    SchemaMigration {
        from: 86,
        to: 87,
