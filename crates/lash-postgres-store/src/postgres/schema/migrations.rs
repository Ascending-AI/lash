...
/// guest asked for (component 96). Component 99 is therefore retained as the
/// refusal-only endpoint and no row targets component 100.
///
...
pub(super) const SCHEMA_MIGRATIONS: &[SchemaMigration] = &[
    ...
    SchemaMigration {
        ...
        // subscription lifecycle column (FIG-1951) and component 100 the
        // stable CHECK constraint names (FIG-3261). No predecessor records
        // existed at any of these moves, so the retained endpoint carries all
        // six: a pre-cutover store is refused at open rather than migrated
