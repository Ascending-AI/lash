...
/// (FIG-3586) adds. Component 122 (FIG-3598) changes the persisted
/// runtime-error vocabulary and moves no relation. No arm targets 122: the
/// current build refuses every predecessor, including 121.
/// Source-shape
...
pub(super) const SCHEMA_MIGRATIONS: &[SchemaMigration] = &[
    ...
    // The immediate predecessor of the retained endpoint 121. A component-120
    // catalog lacks the `lash_turn_parks` table component 121 (FIG-3586)
    // added; component 122 is destructive, so the row is a refusal boundary
