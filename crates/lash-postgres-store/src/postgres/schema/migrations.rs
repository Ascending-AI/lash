...
pub(super) const SCHEMA_MIGRATIONS: &[SchemaMigration] = &[
    // Component 87 is the refusal-only FIG-677 cutover. No row targets it:
    // component-86 stores must be recreated before exact artifact-owner and
    // cleanup-evidence rows can be read.
    // Component 86 combined incompatible component-85 parents as a destructive
    // cutover, so component 85 remains a refusal-only source in this catalog.
    SchemaMigration {
        from: 85,
        to: 86,
