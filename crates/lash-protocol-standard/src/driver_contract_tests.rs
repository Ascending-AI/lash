...
#[test]
fn response_without_tool_calls_checkpoints_to_completion_and_starts_no_tools() {
    // Bounded on purpose: if the driver ever treats a tool-call-free response as
    // a tool-calling one it dispatches an empty tool batch and the turn loops
    // forever. The budget turns that into a fast, readable failure.
    ...
    machine.handle_response(Response::LlmComplete {
        id: llm_id,
        text_streamed: false,
        result: Ok(text_response("final answer")),
    });
    let effects = drain(&mut machine);

    assert!(
        tool_calls(&effects).is_none(),
        "a response with no tool calls must not dispatch tools: {effects:?}"
    );
    ...
    assert!(
        machine.is_done(),
        "the turn completes on the first response"
    );
    assert_eq!(
        ...
        "the assistant message is the turn's finish value"
    );
}

#[test]
fn response_with_tool_calls_dispatches_tools_and_does_not_complete() {
    // The other side of the same predicate: a tool-calling response must not
    // take the completion branch.
    let mut machine = machine(Some(4));
    let effects = drain(&mut machine);
    let llm_id = llm_call_id(&effects);
    ...
    let (_, calls) = tool_calls(&effects)
        .unwrap_or_else(|| panic!("expected dispatched tool calls, got {effects:?}"));
    assert_eq!(
        calls
            .iter()
    ...
    assert!(
        checkpoint(&effects).is_none(),
        "a tool-calling response does not reach the completion checkpoint: {effects:?}"
    );
