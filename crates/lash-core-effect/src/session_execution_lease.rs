...
pub struct SessionExecutionLeaseGuard {
    /// Only the lease port: after acquisition the guard renews and releases
    /// the lane and never touches session state.
    store: Arc<dyn SessionExecutionLeaseStore>,
...
impl SessionExecutionLeaseGuard {
    ...
    pub async fn try_acquire_with_busy_holder(
        ...
        let guard = Self::from_acquisition(
            Arc::clone(&store) as Arc<dyn SessionExecutionLeaseStore>,
            acquisition,
            timings,
            clock,
        );
        store.admit_session_state(&guard.fence()).await?;
...
mod tests {
    ...
    const SESSION_ID: &str = "execution-fence-evidence";

    /// The execution-fence predicate every backend shares traces typed, redacted
    /// decision evidence when a stale holder presents its fence.
    #[tokio::test]
    async fn the_execution_fence_predicate_traces_typed_decision_evidence() {
        let current = SessionExecutionLease {
            session_id: SessionId::from(SESSION_ID),
            owner: crate::LeaseOwnerIdentity::opaque("current-owner", "current-incarnation"),
            executor_id: "current-executor".to_string(),
