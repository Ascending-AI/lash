...
#[expect(clippy::expect_used, reason = "fixture data types and operations are registered once each into a fresh catalog, so registration cannot fail, per each message")]
...
#[expect(clippy::expect_used, reason = "the benchmark program links against the fixture host environment; a failure would break the fixture author's assumption")]
