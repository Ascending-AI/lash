...
fn hard_keyword_reservation_rows_match_the_lexer() {
    let table_names = RESERVED_NAME_POSITIONS
        .iter()
        .filter_map(|(name, positions)| (*positions == ALL_IDENTIFIER_POSITIONS).then_some(*name))
        .collect::<Vec<_>>();
...
fn bare_finish_requires_value() {
    ...
    let err = parse("process p() { finish }").expect_err("bare process finish should be rejected");
    assert!(matches!(err, ParseError::MissingFinishValue { .. }));
}
