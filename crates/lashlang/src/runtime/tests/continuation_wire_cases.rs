...
fn continuation_decode_rejects_an_active_function_without_a_root_frame() {
    ...
    let wire = r#"{"format_version":5,"instruction_pointer":1,"active_function":0,"operand_stack":[],"last_value":{"kind":"unset"},"slots":[],"projected_slots":[],"globals":{"kind":"record","value":[]},"iterator_stack":[],"frame_stack":[],"handler_stack":[],"finally_stack":[],"occurrence_counters":{},"mode":"Process","profile":null,"pending_error_span":null,"instructions_executed":1,"active_execution_elapsed":{"secs":0,"nanos":0},"heap":{"next_id":1,"allocation_counter":0,"live_logical_bytes":0,"size_schedule_version":1,"objects":[]}}"#;
...
fn accept_continuation(wire: &str) {
    serde_json::from_str::<VmContinuation>(wire).expect("shared continuation must decode");
}

#[tokio::test(flavor = "current_thread")]
async fn continuation_decode_accepts_shared_dags_and_rejects_cycles() {
    let empty_list = r#"[{"id":1,"object":{"kind":"list","items":[]}}]"#;
    let empty_bytes = 16;

    // Two durable slots naming one object.
    accept_continuation(&slots_wire(
    ...
    accept_continuation(&restore_and_slot);
}
