...
/// The pending-tool scenario on Restate: the in-process `lash-restate-test`
/// server double under the scenario's seed, the turn inside an engine
/// handler, the tool's await key resolved through the engine's durable wait.
/// Twenty seeds, each proven the same way.
...
async fn pending_tool_completion_proof_runs_on_the_restate_server_double() {
    ...
    println!(
        "pending-tool scenario on the Restate server double: 20 seeds in {:?}",
        started.elapsed()
    );
}
