...
async fn surface_runners(
    ...
    // The memory lane is one SQLite memory backend, every port its own.
    ...
    let memory_effect = memory.effect_host();
...
async fn generated_cross_backend_surface_differential_agrees() {
    ...
    eprintln!(
        "cross-backend generated coverage is bounded: cases={cases} first_seed={runner_seed} \
         ...
         contiguous range; effect_journal_backends=sqlite,postgres (the memory runner's \
         journal is the same SQLite driver, compared through the shared surface)"
    );
