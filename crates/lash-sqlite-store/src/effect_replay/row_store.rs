    ...
    tx.query_row(
        ...
        |row| {
            Ok(EffectGroupRecord {
                ...
                expected_children: usize_from_sql(
                    "RuntimeEffectGroup",
                    "expected_children",
                    row.get(5)?,
                )?,
...
/// fence loss.
fn finalize_miss_cancel_decided(
    tx: &rusqlite::Transaction<'_>,
    fence: &EffectLeaseFence,
) -> rusqlite::Result<bool> {
    let state = tx
...
fn bump_group_rank(tx: &rusqlite::Transaction<'_>, group_key: &str) -> rusqlite::Result<i64> {
    tx.query_row(
