...
async fn durable_core_generation_43_is_refused_at_the_blake3_boundary() {
    ...
    // semantic-boundary receipt identity, 52 mediated plugin state; 53 adds the
    // persisted usage disposition. All are
    ...
    assert_eq!(expected, 53, "the pinned durable-core target changed");
