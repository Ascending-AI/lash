...
async fn unsupported_schema_error_reports_real_versions() {
    ...
    assert!(
        message.contains("schema version 62"),
        "error must report the real expected version 62: {message}"
    );
    assert!(
...
async fn plugin_state_cutover_refuses_snapshot_predecessor_without_mutation() {
    ...
    assert!(
        error.contains("schema version 62") && error.contains("version 51"),
        "{error}"
    );
    let conn = rusqlite::Connection::open(&path).unwrap();
