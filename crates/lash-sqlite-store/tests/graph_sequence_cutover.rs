...
async fn sqlite_41_graph_sequence_shape_is_rejected_without_migration() {
    ...
    assert_eq!(
        error,
        "Error(\"Unsupported lash session schema: this binary supports schema version 42, but the database reports version 41. There is no migration chain — delete the session database and start fresh.\")"
    );
