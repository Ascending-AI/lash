...
const RETAINED_PRIOR_DURABLE_CORE_GENERATION: i32 = 53;
...
async fn sqlite_41_graph_sequence_shape_is_rejected_without_migration() {
    ...
    assert_eq!(
        error,
        "Error(\"Unsupported lash durable core schema: this binary supports schema version 54, but the database reports version 41. There is no migration chain — delete the durable core database and start fresh.\")"
    );
