...
#[tokio::test]
pub(super) async fn restate_recorded_effect_outcome_survives_append_crash_windows() {
    use std::sync::atomic::{AtomicUsize, Ordering};

    let context = Arc::new(ReplayableRecordingContext::default());
    let controller = RestateRuntimeEffectController::new_for_test(context.clone());
    let registry = process_registry();
    let process_id = ProcessId::from("restate-effect-summary-crash-windows");
    registry
        .register_process(external_registration(process_id.as_str()))
        .await
        .expect("register effect-summary process");
    let envelope = RuntimeEffectEnvelope::new(
        runtime_invocation(
            RuntimeEffectKind::LanguageRuntimeValue,
            "restate-summary-value",
        ),
        ...
        .execute_effect(
            ...
            RuntimeEffectLocalExecutor::testing(move |_| async move {
                ...
                Ok(RuntimeEffectOutcome::LanguageRuntimeValue {
                    value: serde_json::json!(887),
                })
            }),
        )
        .await
        .expect("journal first effect result");
    assert!(matches!(
        first,
    ...
    assert!(
        registry
            ...
            .await
            .expect("read pre-append log")
            .is_empty(),
        "interruption after effect journal and before append leaves no summary row"
    );

    let occurrence = lash_core::ProcessEffectSummaryOccurrence::new(
    ...
    for expected_events in [1, 1, 2] {
        context.start_replay();
        let replay_counter = Arc::clone(&executions);
        let replayed = controller
            ...
            .await
            .expect("replay journaled effect without resampling");
        assert!(matches!(
            replayed,
            RuntimeEffectOutcome::LanguageRuntimeValue { value } if value == serde_json::json!(887)
        ));
        let receipt = registry
            .append_event(&process_id, occurrence.append_request())
            .await
            .expect("append or replay recorded outcome");
        ...
        if expected_events == 2 {
            registry
                ...
                .await
                .expect("terminalise after effect replay");
        }
        assert_eq!(
            registry
                .full_event_window(&process_id, 0)
                .await
                .expect("read effect-summary log")
                .len(),
            expected_events
        );
    }
    ...
    let terminal_replay = registry
        .append_event(&process_id, occurrence.append_request())
        .await
        .expect("recorded effect may replay after terminalisation");
    assert_eq!(terminal_replay.event.sequence, 1);
    assert_eq!(
        registry
            .full_event_window(&process_id, 0)
            .await
            .expect("read terminal replay log")
            .len(),
        2
    );
}
