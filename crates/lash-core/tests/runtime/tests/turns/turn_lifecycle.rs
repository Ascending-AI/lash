...
pub(super) async fn post_commit_restore_failure_is_a_diagnostic_and_forces_reload() {
    ...
    let mut runtime = runtime_with_plugins_and_tools_and_host_and_store(
        ...
        Arc::new(TerminalControlTool {
            controls: vec![lash_core::ToolControl::SwitchAgentFrame {
                frame_key: lash_core::FrameKey::from_caller_material("restore-failure-frame")
    ...
    assert!(committed.errors.iter().any(|issue| {
        issue.code == Some(lash_core::TurnFailureCode::ProtocolRestoreSession.into())
...
pub(super) async fn follow_on_capture_failure_returns_the_committed_frame_and_handoff_is_retry_safe()
 {
    ...
    let mut runtime = runtime_with_plugins_and_tools_and_host_and_store(
        ...
        Arc::new(TerminalControlTool {
            controls: vec![lash_core::ToolControl::SwitchAgentFrame {
                frame_key: lash_core::FrameKey::from_caller_material("capture-failure-frame")
                    .expect("non-empty caller material"),
                initial_nodes: Vec::new(),
...
pub(super) async fn continue_as_frame_rotation_reconciles_newly_advertised_tool() {
    ...
    let tools: Arc<dyn lash_core::ToolProvider> = Arc::new(FrameRotatingDynamicTool {
    ...
    assert!(
        !post_rotation_state
            .get(&lash_core::ToolId::from("tool:curated_before_rotation"))
            .expect("curated entry survives rotation")
            .is_member()
    );
    assert!(
        post_rotation_state
            .get(&lash_core::ToolId::from("tool:hidden_after_rotation"))
...
#[async_trait::async_trait]
impl lash_core::AwaitEventResolver for JournalReplayEffectController {
...
#[async_trait::async_trait]
impl lash_core::RuntimeEffectController for JournalReplayEffectController {
...
pub(super) fn request_contains_text(
