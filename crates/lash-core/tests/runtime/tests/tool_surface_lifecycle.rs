...
async fn cold_resume_discovers_curated_live_surface_and_persists_it_without_flapping() {
    ...
    assert!(
        !resumed_state
            .get(&lash_core::ToolId::from(original.id))
    ...
    assert!(
        resumed_state
            .get(&lash_core::ToolId::from(discovered.id))
...
async fn session_fork_discovers_live_tools_and_preserves_curation_and_hidden_policy() {
    ...
    parent_state
        .set_membership(&lash_core::ToolId::from(curated.id), false)
    ...
    assert!(
        !child_state
            .get(&lash_core::ToolId::from(curated.id))
...
async fn orphan_lifecycle_rebinds_by_id_and_supersedes_same_name_without_duplicates() {
    ...
    assert!(!replaced.contains(&lash_core::ToolId::from(original.id)));
    assert!(
        replaced
            .get(&lash_core::ToolId::from(replacement.id))
...
async fn public_apply_tool_state_round_trip_keeps_delta_and_generation_fencing() {
    ...
    edited
        .set_membership(&lash_core::ToolId::from(first.id), false)
    ...
    assert!(
        !applied
            .get(&lash_core::ToolId::from(first.id))
            .expect("first entry retained")
            .is_member()
    );
    assert!(
        applied
            .get(&lash_core::ToolId::from(second.id))
