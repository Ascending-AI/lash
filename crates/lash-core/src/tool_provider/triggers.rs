    ...
    pub async fn emit(
                ...
                == crate::EffectJournalAddressing::OrdinalAddressed
        {
            return Err(PluginError::Session(
                "ToolContext::triggers().emit() is unavailable inside an atomic tool attempt on ordinal-addressed journal tiers; return a ToolIntent::EmitTrigger declaration so the emission follows the committed attempt, or emit the trigger from a process step"
                    .to_string(),
            ));
        }
...
mod tests {
    ...
    async fn trigger_emit_is_rejected_inside_controller_owned_atomic_tool_attempt() {
        ...
        assert_eq!(
            error.to_string(),
            "plugin session error: ToolContext::triggers().emit() is unavailable inside an atomic tool attempt on ordinal-addressed journal tiers; return a ToolIntent::EmitTrigger declaration so the emission follows the committed attempt, or emit the trigger from a process step"
        );
    }
}
