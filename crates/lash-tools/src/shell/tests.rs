...
impl lash_core::ProcessService for TestProcessService {
    ...
    async fn complete_external(
        ...
        match self.registry.get_process(process_id).await? {
            Some(record) if record.disposition != lash_core::RecoveryContract::ExternallyOwned => {
                return Err(PluginError::Session(format!(
...
async fn cancelled_detached_spawn_cannot_escape_without_a_durable_audit_row() {
    ...
    let task =
        tokio::spawn(async move { run_with_context(&shell, "start_command", &args, &ctx).await });
...
fn shell_definitions_are_compact_and_non_empty() {
    ...
    assert_eq!(
        defs.iter()
            .filter(
                |definition| definition.manifest.activation == lash_core::ToolActivation::Internal
            )
