...
fn run_regexp_named_capture_groups_tc(tc: TestConfig) {
    ...
    // This pattern should fail in unicode mode, because there is a backreference without a
    // capture group.
    // If the `\]` is not handled correctly in the parser, the following `(.)` may be parsed as
    // a capture group.
...
mod utf16_tests {
    ...
    fn test_utf16_byte_sequences_tc(tc: TestConfig) {
        // Regress emits byte sequences for e.g.
    ...
    fn test_ucs2_surrogates() {
        // TODO: we improperly match this in UTF-16 mode, because our API is kind of bad.
        // We ought to infer UTF-16 vs UCS2 from the "u" flag, matching the JS spec.
