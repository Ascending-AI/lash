...
impl<'a, Input: InputIndexer> MatchAttempter<'a, Input> {
    ...
    pub(crate) fn new_with_fuel(re: &'a CompiledRegex, entry: Input::Position, fuel: u64) -> Self {
        let mut matcher = Self::new(re, entry);
        matcher.fuel = Some(fuel);
        matcher
    }

    #[inline(always)]
    fn charge_step(&mut self) -> bool {
        let Some(remaining) = self.fuel.as_mut() else {
            return true;
        };
        if *remaining == 0 {
            self.exhausted = true;
...
impl<Input: InputIndexer> exec::FallibleMatchProducer for BacktrackExecutor<'_, Input> {
    fn try_next_match(
        &mut self,
        pos: Input::Position,
        next_start: &mut Option<Input::Position>,
    ) -> Result<Option<Match>, exec::MatchError> {
        let found = <Self as exec::MatchProducer>::next_match(self, pos, next_start);
        if self.matcher.exhausted {
            Err(exec::MatchError::Exhausted)
        } else {
            Ok(found)
        }
    }
    ...
    fn try_next_match_anchored(
        ...
        let found = self.next_match_anchored(pos, next_start);
        if self.matcher.exhausted {
            Err(exec::MatchError::Exhausted)
        } else {
            Ok(found)
