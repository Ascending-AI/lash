...
#[cfg(feature = "utf16")]
use crate::indexing::{Ucs2Input, Utf16Input};
...
impl Regex {
    ...
    /// Returns a match iterator with a deterministic shared execution budget.
    pub fn try_find_from<'r, 't>(
        &'r self,
        text: &'t str,
        start: usize,
        fuel: u64,
    ) -> exec::TryMatches<super::classicalbacktrack::BacktrackExecutor<'r, indexing::Utf8Input<'t>>>
    {
        ...
        exec::TryMatches::new(
            super::classicalbacktrack::BacktrackExecutor::new(
                input,
                MatchAttempter::new_with_fuel(&self.cr, input.left_end(), fuel),
            ),
            start,
        )
    }
    ...
    /// UTF-16 matching with the same deterministic fuel facility as UTF-8.
    #[cfg(feature = "utf16")]
    pub fn try_find_from_utf16<'r, 't>(
        &'r self,
        text: &'t [u16],
        start: usize,
        fuel: u64,
    ...
    /// UTF-16 matching anchored at exactly `start`, with deterministic fuel.
    #[cfg(feature = "utf16")]
    pub fn try_find_from_utf16_anchored<'r, 't>(
        &'r self,
        text: &'t [u16],
        start: usize,
        fuel: u64,
    ...
    /// UCS-2 matching with the same deterministic fuel facility as UTF-8.
    #[cfg(feature = "utf16")]
    pub fn try_find_from_ucs2<'r, 't>(
        &'r self,
        text: &'t [u16],
        start: usize,
        fuel: u64,
    ...
    /// UCS-2 matching anchored at exactly `start`, with deterministic fuel.
    #[cfg(feature = "utf16")]
    pub fn try_find_from_ucs2_anchored<'r, 't>(
        &'r self,
        text: &'t [u16],
        start: usize,
        fuel: u64,
...
mod deterministic_fuel_tests {
    ...
    fn utf8_utf16_and_ucs2_report_the_same_typed_exhaustion() {
        ...
        assert!(matches!(
            regex.try_find_from("a", 0, 0).next(),
            Some(Err(MatchError::Exhausted))
        ));

        #[cfg(feature = "utf16")]
        {
            let input = [b'a' as u16];
            assert!(matches!(
                regex.try_find_from_utf16(&input, 0, 0).next(),
                Some(Err(MatchError::Exhausted))
            ));
            assert!(matches!(
                regex.try_find_from_ucs2(&input, 0, 0).next(),
                Some(Err(MatchError::Exhausted))
            ));
        }
    }
