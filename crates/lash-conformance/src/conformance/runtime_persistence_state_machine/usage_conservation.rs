...
pub(super) fn record_usage(
    ...
    lash_core::testing::conformance_support::record_token_usage_shared(
        &model.pending_usage,
        &entry.source,
        &entry.model,
        &entry.usage,
...
pub(super) fn stage_usage(
    ...
    let staged = lash_core::testing::conformance_support::stage_token_ledger_shared(
        &model.pending_usage,
        &operation,
    )
...
fn pending_usage_snapshot(
    model: &ReferenceModel,
) -> Vec<lash_core::testing::conformance_support::PendingTokenLedgerEntry> {
