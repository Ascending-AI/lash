...
impl EffectHost for ParentEndFaultHost {
    fn turn_control_binding_id(&self) -> String {
        "parent-end-fault-host".to_string()
    }
...
async fn segmented_registration(
    ...
    lashlang::LashlangArtifactStore::publish_module_artifact(
        ...
        &lash_core::ArtifactOwner::host("process-parent-atomicity"),
        &linked.artifact,
    )
    .await
...
async fn public_process_parents_are_literal_and_crash_atomic_on_postgres() {
    ...
    // Semantic hash v8 and its complete process output deliberately rekey the
