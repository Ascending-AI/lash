...
async fn redelivered_cancel_requests_the_same_cancellation_once() -> Result<()> {
    ...
    // content under the same identity" can only mean a different target. On
    // this tier that is *not* refused: the cancel fence lives on the target
    // record (`runtime/process/validation.rs:460-469` — first writer wins,
    // matching requester replays), and nothing binds a replay key to the
    // target it first named. Submitting the bound identity against a second
    // process therefore cancels that second process, one realization each,
    // rather than being refused. The runtime-owned tier does bind identity to
    // target, from its pre-realization submission ledger, which is what
    // `runtime_owned_identity_is_bound_before_a_different_target_is_submitted`
    // proves. Asserting the controller-owned behaviour here would bless it, so
    // this test stops at the single-realization law the ticket asks for and
    // the gap is called out in the PR instead.
    Ok(())
}
