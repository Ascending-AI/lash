...
use crate::plugins::{
    PluginError, PluginExtensionContribution, PluginFactory, PluginRegistrar, PluginSessionContext,
    SessionPlugin,
};
use crate::rlm::{
...
/// each call.
///
/// The suite builds every core over that backend, its RLM protocol factory
/// included (the backend supplies the Lashlang artifact store), and drives
/// and awaits processes through the core's process registry, so the backend
...
where
    F: Fn() -> Fut,
    Fut: std::future::Future<Output = lash_core::Backend>,
{
...
fn base_builder(backend: &lash_core::Backend) -> LashCoreBuilder {
...
fn build_core(backend: &lash_core::Backend) -> LashCore {
...
async fn await_success(
    ...
    let outcome = tokio::time::timeout(Duration::from_secs(10), awaiter.await_terminal(process_id))
        .await
...
async fn reopen_restores_trigger_registry_state(backend: lash_core::Backend) {
...
async fn worker_runs_trigger_started_lashlang_process_after_restart(backend: lash_core::Backend) {
    let core = build_core(&backend);
    let registry = core.process_registry();
    open_mutate_and_restart(&core, None, &registry).await;
...
async fn trigger_triggered_process_wake_provenance_survives_restart(backend: lash_core::Backend) {
    let core = build_core(&backend);
    let registry = core.process_registry();
            ...
            loop {
                match commits.next().await {
                    Some(Ok(crate::observe::SessionObservationStreamItem::Event(event))) => {
                        ...
                        if let lash_core::SessionObservationEventPayload::Committed { read_view } =
                            &event.payload
