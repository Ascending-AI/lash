...
impl LashCore {
    ...
    pub async fn delete_session(
        ...
        lash_core::drive::close_session(&context)
            .await
            .map_err(|error| match error {
                lash_core::drive::SessionCloseError::Store(error) => EmbedError::from(error),
                lash_core::drive::SessionCloseError::Runtime(error) => EmbedError::from(error),
            })?;
