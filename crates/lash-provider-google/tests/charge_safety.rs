...
use lash_core::llm::types::{
    LlmEventSender, LlmMessage, LlmRequest, LlmRole, LlmToolChoice, LlmToolSpec, LlmUsage,
};
use lash_core::provider::{
    ProviderHandle, ProviderOptions, ProviderReliability, StreamTermination,
};
...
#[test]
fn charge_safety_google_escaped_content_refuses_retry_with_typed_reason() {
    let body = "data: {\"response\":{\"candidates\":[{\"content\":{\"parts\":[{\"text\":\"partial\"}]}}]}}\n\n";
...
#[test]
fn charge_safety_google_truly_empty_partial_schedules_empty_stream_retry() {
    let body = "data: {\"response\":{\"candidates\":[{\"content\":{\"parts\":[]}}]}}\n\n";
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .expect("test runtime");
    ...
    let partial = failure
        ...
        .partial_response
        .as_deref()
        .expect("Google empty-stream failure carries a partial response");
