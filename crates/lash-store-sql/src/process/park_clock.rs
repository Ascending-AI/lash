//! `process_park_clock`: the process park feed's single-row sequence
//! (FIG-3659 NOW-B).
//!
//! The process twin of `turn_park_clock`, in the registry's database. One
//! row, two counters, and no shared statement at all: every operation over
//! this table forks. `current_seq` is the sequence every feed event is
//! allocated from, which makes `process_park_events.seq` order equal commit
...
pub const TABLE: &str = "process_park_clock";
