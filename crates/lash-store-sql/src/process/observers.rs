//! `process_observers`: which sessions observe which process.
//!
//! Two columns, both of them key. There is no projection of this table wider
...
crate::statements! {
    ...
    pub struct ObserverStatements @ "process_observer" {
        ...
        /// PostgreSQL.
        insert = "INSERT INTO process_observers (session_id, process_id)
                             VALUES (?1, ?2)";
        ...
        /// The sessions observing process `?1`.
        list_sessions_for_process = "SELECT session_id FROM process_observers
                             WHERE process_id = ?1
                             ORDER BY session_id";
    }
}
