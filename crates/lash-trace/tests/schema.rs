...
#[test]
fn trace_schema_version_is_pinned_at_35() {
    ...
    assert_eq!(lash_trace::TRACE_SCHEMA_VERSION, 35);
}
...
#[test]
fn predecessor_trace_schema_is_refused_before_event_decode() {
    let predecessor = r#"{"schema_version":19,"id":"v19","timestamp":"2026-09-06T00:00:00+00:00","context":{},"type":"turn_started"}"#;
    let error = serde_json::from_str::<TraceRecord>(predecessor)
        .expect_err("schema-19 records must be refused before event decode");
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 19; expected 35"
    );
}

/// FIG-1272: schema 20 is the generation before the context-overflow turn
/// failure reason, and is refused by the same gate.
#[test]
fn trace_schema_20_is_refused_before_event_decode() {
    let predecessor = r#"{"schema_version":20,"id":"v20","timestamp":"2026-09-13T00:00:00+00:00","context":{},"type":"turn_started"}"#;
    let error = serde_json::from_str::<TraceRecord>(predecessor)
        .expect_err("schema-20 records must be refused before event decode");
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 20; expected 35"
    );
}

#[test]
fn immediate_predecessor_trace_schema_32_is_refused_before_event_decode() {
    let predecessor = r#"{"schema_version":32,"id":"v32","timestamp":"2026-09-23T00:00:00+00:00","context":{},"type":"turn_started"}"#;
    let error = serde_json::from_str::<TraceRecord>(predecessor)
        .expect_err("schema-32 records must be refused before event decode");
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 32; expected 35"
    );
}

/// FIG-3571: schema 33 keyed language observations by the retired node ids.
#[test]
fn immediate_predecessor_trace_schema_33_is_refused_before_event_decode() {
    let predecessor = r#"{"schema_version":33,"id":"v33","timestamp":"2026-09-24T00:00:00+00:00","context":{},"type":"turn_started"}"#;
    let error = serde_json::from_str::<TraceRecord>(predecessor)
        .expect_err("schema-33 records must be refused before event decode");
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 33; expected 35"
    );
}
...
fn schema_14_trace_is_refused_before_typed_cell_failure_decode() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 14; expected 35"
    );
}
...
fn documented_trace_record_decode_rejects_schema_3_before_payload_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 3; expected 35"
    );
...
fn schema_8_exec_protocol_record_is_refused_before_old_envelope_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 8; expected 35"
    );
}
...
fn schema_9_exec_completion_record_is_refused_before_old_projection_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 9; expected 35"
    );
}
...
fn schema_10_retry_attempt_record_is_refused_before_charge_safety_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 10; expected 35"
    );
}
...
fn schema_11_retry_attempt_record_is_refused_before_attempt_evidence_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 11; expected 35"
    );
}

#[test]
fn schema_12_record_is_refused_before_attachment_event_interpretation() {
    let stored_v12 = r#"{"schema_version":12,"id":"v12","timestamp":"2026-08-30T09:00:00+00:00","context":{},"type":"turn_started"}"#;
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 12; expected 35"
    );
}
...
fn schema_13_llm_completion_without_request_model_is_refused_before_response_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 13; expected 35"
    );
}
...
fn historical_v4_reader_refuses_v5_before_interpreting_new_closed_enum_variant() {
    ...
    let wire = current_wire.replacen("\"schema_version\":35", "\"schema_version\":5", 1);
    assert_eq!(
...
fn historical_v5_reader_refuses_v6_provider_replay_dropped_before_interpreting_variant() {
    ...
    let wire = current_wire.replacen("\"schema_version\":35", "\"schema_version\":6", 1);
    assert_eq!(
...
fn historical_v6_reader_refuses_v7_language_execution_before_interpreting_variant() {
    ...
    let wire = current_wire.replacen("\"schema_version\":35", "\"schema_version\":7", 1);
    assert_eq!(
...
fn historical_v7_reader_refuses_v8_turn_outcome_before_interpreting_payload() {
    ...
    let wire = current_wire.replacen("\"schema_version\":35", "\"schema_version\":8", 1);
    assert_eq!(
...
fn current_reader_refuses_a_stored_v7_turn_completed_record() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 7; expected 35",
    ...
    // stale reader sees.
    let forced_current = stored_v7.replacen("\"schema_version\":7", "\"schema_version\":35", 1);
...
#[test]
fn standard_compaction_events_pin_decision_payloads() {
    let events = [
        TraceEvent::CompactionNeeded {
            used_tokens: 30_000,
            max_context_tokens: 40_000,
...
#[test]
fn exec_code_completed_failure_is_structured() {
    let event = TraceEvent::ExecCodeCompleted {
        duration_ms: 12,
        output: String::new(),
        output_chars: 0,
        observation_count: 0,
        ...
        error: Some(lash_trace::CellFailure::new(
            lash_trace::CellFailureKind::Host,
    ...
    let json = serde_json::to_value(event).expect("serialize exec completion");
    assert_eq!(
        json["error"],
        json!({ "kind": "host", "message": "artifact store unavailable" })
    );
}
...
fn jsonl_round_trip_preserves_records() {
    ...
    for record in &parsed {
        assert_eq!(record.schema_version, 35);
    }
...
fn rlm_compile_link_outcomes_have_pinned_wire_tags() {
    for (outcome, expected) in [
        ...
        assert_eq!(
            serde_json::from_value::<TraceEvent>(expected).unwrap(),
            event
        );
    }
}
...
fn published_trace_record_schema_accepts_every_event_and_payload_sample() {
    let validator = published_schema(include_str!(
        "../../../schemas/host/trace-record/v35.schema.json"
    ))
...
fn published_graph_schema_accepts_a_folded_snapshot_and_enforces_its_row() {
    let validator = published_schema(include_str!(
        "../../../schemas/host/trace-lashlang-graph/v35.schema.json"
    ))
    .expect("published trace schema");
