...
#[test]
fn trace_schema_version_is_pinned_at_13() {
    ...
    assert_eq!(lash_trace::TRACE_SCHEMA_VERSION, 13);
}
...
fn documented_trace_record_decode_rejects_schema_3_before_payload_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 3; expected 13"
    );
...
fn schema_8_exec_protocol_record_is_refused_before_old_envelope_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 8; expected 13"
    );
}
...
fn schema_9_exec_completion_record_is_refused_before_old_projection_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 9; expected 13"
    );
}
...
fn schema_10_retry_attempt_record_is_refused_before_charge_safety_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 10; expected 13"
    );
}
...
fn historical_v4_reader_refuses_v5_before_interpreting_new_closed_enum_variant() {
    ...
    let wire = current_wire.replacen("\"schema_version\":13", "\"schema_version\":5", 1);
    assert_eq!(
...
fn historical_v5_reader_refuses_v6_provider_replay_dropped_before_interpreting_variant() {
    ...
    let wire = current_wire.replacen("\"schema_version\":13", "\"schema_version\":6", 1);
    assert_eq!(
...
fn historical_v6_reader_refuses_v7_language_execution_before_interpreting_variant() {
    ...
    let wire = current_wire.replacen("\"schema_version\":13", "\"schema_version\":7", 1);
    assert_eq!(
...
fn historical_v7_reader_refuses_v8_turn_outcome_before_interpreting_payload() {
    ...
    let wire = current_wire.replacen("\"schema_version\":13", "\"schema_version\":8", 1);
    assert_eq!(
...
fn current_reader_refuses_a_stored_v7_turn_completed_record() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 7; expected 13",
    ...
    // stale reader sees.
    let forced_v13 = stored_v7.replacen("\"schema_version\":7", "\"schema_version\":13", 1);
    let error = serde_json::from_str::<TraceRecord>(&forced_v13)
