...
#[test]
fn trace_schema_version_is_pinned_at_11() {
    ...
    assert_eq!(lash_trace::TRACE_SCHEMA_VERSION, 11);
}
...
fn documented_trace_record_decode_rejects_schema_3_before_payload_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 3; expected 11"
    );
...
fn schema_8_exec_protocol_record_is_refused_before_old_envelope_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 8; expected 11"
    );
}
...
fn schema_9_exec_completion_record_is_refused_before_old_projection_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 9; expected 11"
    );
}

#[test]
fn schema_10_retry_attempt_record_is_refused_before_charge_safety_interpretation() {
    let stored_v10 = r#"{"schema_version":10,"id":"v10-retry","timestamp":"2026-08-29T09:00:00+00:00","context":{},"type":"llm_call_failed","error":{"message":"rate limited","retryable":true},"attempts":[{"ordinal":1,"outcome":"failed","duration_ms":10,"charge_safety":{"outcome":"authorized","tokens_at_stake":42,"attempt_number":1}}]}"#;
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 10; expected 11"
    );
}
...
fn historical_v4_reader_refuses_v5_before_interpreting_new_closed_enum_variant() {
    ...
    let wire = current_wire.replacen("\"schema_version\":11", "\"schema_version\":5", 1);
    assert_eq!(
...
fn historical_v5_reader_refuses_v6_provider_replay_dropped_before_interpreting_variant() {
    ...
    let wire = current_wire.replacen("\"schema_version\":11", "\"schema_version\":6", 1);
    assert_eq!(
...
fn historical_v6_reader_refuses_v7_language_execution_before_interpreting_variant() {
    ...
    let wire = current_wire.replacen("\"schema_version\":11", "\"schema_version\":7", 1);
    assert_eq!(
...
fn historical_v7_reader_refuses_v8_turn_outcome_before_interpreting_payload() {
    ...
    let wire = current_wire.replacen("\"schema_version\":11", "\"schema_version\":8", 1);
    assert_eq!(
...
fn current_reader_refuses_a_stored_v7_turn_completed_record() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 7; expected 11",
    ...
    // stale reader sees.
    let forced_v11 = stored_v7.replacen("\"schema_version\":7", "\"schema_version\":11", 1);
    let error = serde_json::from_str::<TraceRecord>(&forced_v11)
