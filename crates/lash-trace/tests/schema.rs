...
fn predecessor_trace_schema_is_refused_before_event_decode() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 19; expected 21"
    );
}
...
fn immediate_predecessor_trace_schema_20_is_refused_before_event_decode() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 20; expected 21"
    );
}
...
fn schema_14_trace_is_refused_before_typed_cell_failure_decode() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 14; expected 21"
    );
}
...
fn documented_trace_record_decode_rejects_schema_3_before_payload_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 3; expected 21"
    );
...
fn schema_8_exec_protocol_record_is_refused_before_old_envelope_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 8; expected 21"
    );
}
...
fn schema_9_exec_completion_record_is_refused_before_old_projection_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 9; expected 21"
    );
}
...
fn schema_10_retry_attempt_record_is_refused_before_charge_safety_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 10; expected 21"
    );
}
...
fn schema_11_retry_attempt_record_is_refused_before_attempt_evidence_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 11; expected 21"
    );
}
...
fn schema_12_record_is_refused_before_attachment_event_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 12; expected 21"
    );
}
...
fn schema_13_llm_completion_without_request_model_is_refused_before_response_interpretation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 13; expected 21"
    );
}
...
fn historical_v4_reader_refuses_v5_before_interpreting_new_closed_enum_variant() {
    ...
    let wire = current_wire.replacen("\"schema_version\":21", "\"schema_version\":5", 1);
    assert_eq!(
...
fn historical_v5_reader_refuses_v6_provider_replay_dropped_before_interpreting_variant() {
    ...
    let wire = current_wire.replacen("\"schema_version\":21", "\"schema_version\":6", 1);
    assert_eq!(
...
fn historical_v6_reader_refuses_v7_language_execution_before_interpreting_variant() {
    ...
    let wire = current_wire.replacen("\"schema_version\":21", "\"schema_version\":7", 1);
    assert_eq!(
...
fn historical_v7_reader_refuses_v8_turn_outcome_before_interpreting_payload() {
    ...
    let wire = current_wire.replacen("\"schema_version\":21", "\"schema_version\":8", 1);
    assert_eq!(
...
fn current_reader_refuses_a_stored_v7_turn_completed_record() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported trace schema version 7; expected 21",
    ...
    // stale reader sees.
    let forced_current = stored_v7.replacen("\"schema_version\":7", "\"schema_version\":21", 1);
...
fn retry_attempts_are_optional_additive_event_fields() {
    let attempts = Some(vec![
        lash_trace::TraceRetryAttempt {
            ...
            usage: Some(token_usage_sample()),
            usage_disposition: Some("reported".to_string()),
        },
        lash_trace::TraceRetryAttempt {
            ordinal: 2,
...
fn jsonl_round_trip_preserves_records() {
    ...
    for record in &parsed {
        assert_eq!(record.schema_version, 21);
    }
