...
/// FIG-3607: schema 35 keyed process observations by a reusable process name
/// and its incarnation; a minted id names the whole lifetime under 36.
#[test]
fn immediate_predecessor_trace_schema_35_is_refused_before_event_decode() {
    let predecessor = r#"{"schema_version":35,"id":"v35","timestamp":"2026-09-25T00:00:00+00:00","context":{},"type":"turn_started"}"#;
    let error = serde_json::from_str::<TraceRecord>(predecessor)
...
#[test]
fn language_execution_identity_generation_is_the_attempt() {
    // A minted process id already names the lifetime (ADR 0107), so the
    // attempt alone is the generation.
    let mut value = serde_json::to_value(lashlang_identity()).expect("encode identity");
    value["attempt"] = json!(2);
    let identity = serde_json::from_value::<TraceLanguageExecutionIdentity>(value)
        .expect("an attempt alone is a generation");
    assert_eq!(identity.attempt(), Some(2));
    assert!(identity.graph_key().ends_with(":attempt:2"));
}
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event": {
                ...
                "identity": {
                    "scope": { "session_id": "s1" },
                    "subject": { "type": "process", "process_id": lash_sansio::ProcessId::fixture("p1") },
    ...
    let variants = vec![
        ...
        TraceLanguageExecutionPayload::ChildStarted {
            ...
            child: TraceLanguageChildExecution {
                scope: TraceRuntimeScope::new("s1"),
                process_id: lash_sansio::ProcessId::fixture("p2"),
                attempt: Some(2),
