...
fn a_lifted_body_captures_immutable_cell_locals_and_refuses_mutable_ones() {
    ...
    println!(
        "probe-a={:?}",
        lash_typescript::parse(r#" const spend = async (tick: unknown) => { return tick }; "#)
            .map(|p| p.declarations.len())
    );
    println!(
        "probe-b={:?}",
        lash_typescript::parse(
            r#" const budget = 40; const spend = async (tick: unknown) => { return budget }; "#
        )
        .map(|p| p.declarations.len())
    );
    println!("probe-d={:?}", lash_typescript::parse(r#" const budget = 40; const spend = async (tick: unknown) => { console.log(budget); return budget; }; "#).map(|p| p.declarations.len()));
