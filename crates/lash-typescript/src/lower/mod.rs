...
impl Lowerer {
    ...
    fn array_binding_contents(&self, init: &Expr) -> Option<ArrayContents> {
        ...
        Ok(match stmt {
            ...
            Stmt::Var { kind, declarations } => {
                ...
                for declaration in declarations {
                    ...
                    if let Some(name) = process_name
                        && *kind == VarKind::Const
                        && let Some(init) = declaration.init.as_ref()
                        && let Some(contents) = self.array_binding_contents(init)
                    {
                        self.set_role(name, BindingRole::ArrayValue(contents))?;
                    }
