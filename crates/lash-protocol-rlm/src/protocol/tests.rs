...
/// Leg 1 of the retry-hygiene triple: the repair turn starts from empty
/// accumulation.
///
...
/// as output of a program that never ran. The driver gets this by rebuilding
/// the whole state rather than clearing fields, so there is no field to forget:
/// pinned here because "rebuilt from `default()`" is a property of one line
/// that a later refactor could quietly turn into a reuse.
#[test]
fn the_repair_turn_starts_from_empty_accumulation() {
    use super::state::{RlmDriverState, rlm_driver_state};

    let dirty = RlmDriverState {
        output: vec!["printed before the failure".to_string()],
        exec_error: Some("the failure".to_string()),
        executed_code: Some("the failed cell".to_string()),
        calls_omitted: 3,
        ..Default::default()
    };
    let encoded = serde_json::to_value(rlm_driver_state(dirty)).expect("state encodes");
    assert!(
        encoded.to_string().contains("printed before the failure"),
        "the fixture must actually be dirty, or the check below is vacuous"
    );

    let fresh = RlmDriverState::default();
    assert!(fresh.output.is_empty());
    assert!(fresh.exec_error.is_none());
    assert!(fresh.executed_code.is_none());
    assert!(fresh.calls.is_empty());
    assert_eq!(fresh.calls_omitted, 0);
    assert!(fresh.prose.is_empty());
    assert!(fresh.reasoning.is_empty());
    assert!(fresh.images.is_empty());
    assert!(fresh.terminal_finish.is_none());
}
