//! The cell vocabulary the scenarios are written in, and its rendering.
//!
//! A cross-cell law is a statement about a *sequence of cells*, not about
//! their TypeScript spelling, so the scenarios name cells with [`Cell`]
//! and this module renders one into TypeScript source, so a scenario reads as
//! the cell sequence it is rather than as a wall of program text.
//!
...
impl Literal {
    fn render(&self) -> String {
        match self {
            ...
            Self::Nested { scalar, items } => format!(
                "{{ scalar: {}, items: [{}] }}",
...
fn render_number(value: f64) -> String {
    // An integral float reads as the number it is, and the JSON
...
pub(crate) enum Cell {
    ...
    /// cells therefore grows it into a new name, and shadowing gets
    ...
    RuntimeError,
    /// A cell refused as outside the language, as distinct from a
...
impl Cell {
    ...
    /// The source this cell takes.
    pub(crate) fn render(&self) -> String {
        self.render_typescript()
    }

    fn render_typescript(&self) -> String {
        match self {
            Self::Bind { name, value } => {
                format!("const {name} = {};", value.render())
            }
            ...
            Self::Refusal => "class NotASupportedConstruct {}".to_string(),
        }
    }
}
