...
/// Refusal witness (FIG-2962/FIG-2963): the generation-72 decoder rejects its
/// immediate predecessor before attempting to decode the envelope body.
#[test]
fn immediate_predecessor_remote_protocol_generation_71_is_refused() {
    const PREDECESSOR: u32 = 71;
    assert_eq!(
        PREDECESSOR + 1,
    ...
    let error = decode_empty_envelope(PREDECESSOR)
        .expect_err("generation-71 remote envelope must be refused");
    assert!(matches!(
        error,
...
fn protocol_62_session_filter_is_refused_before_removed_field_decode() {
    ...
    assert_eq!(
        ...
        .expect("serialize canonical version-72 filter"),
        serde_json::json!({
            "protocol_version": 72,
...
#[test]
fn remote_protocol_71_session_filter_refuses_retired_session_id() {
    let wire = br#"{"protocol_version":72,"session_id":"session-blue"}"#;
    let error = Envelope::<RemoteTriggerSubscriptionFilter>::decode_json(wire)
        .expect_err("version-72 filter must reject the retired session_id field");
    assert!(matches!(error, RemoteProtocolError::MessageDecode(_)));
    assert!(error.to_string().contains("session_id"), "{error}");
}

#[test]
fn remote_protocol_71_session_filter_refuses_nested_duplicate_fields() {
    let wire = br#"{"protocol_version":72,"target":{"value":1,"value":2}}"#;
    let error = Envelope::<RemoteTriggerSubscriptionFilter>::decode_json(wire)
        .expect_err("version-72 envelope must preserve nested duplicate-field rejection");
    assert!(matches!(error, RemoteProtocolError::MessageDecode(_)));
    assert!(
...
fn remote_process_dtos_json_round_trip() {
    assert_eq!(REMOTE_PROTOCOL_VERSION, 72, "remote DTO wire-shape pin");
