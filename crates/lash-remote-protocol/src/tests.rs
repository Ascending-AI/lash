...
/// Refusal witness (FIG-2996): the generation-82 decoder rejects its immediate
...
#[test]
fn immediate_predecessor_remote_protocol_generation_81_is_refused() {
    const PREDECESSOR: u32 = 81;
    assert_eq!(
        PREDECESSOR + 1,
    ...
    let error = decode_empty_envelope(PREDECESSOR)
        .expect_err("generation-81 remote envelope must be refused");
    assert!(matches!(
        error,
...
fn protocol_62_session_filter_is_refused_before_removed_field_decode() {
    ...
    assert_eq!(
        ...
        .expect("serialize canonical version-82 filter"),
        serde_json::json!({
            "protocol_version": 82,
...
#[test]
fn remote_protocol_82_session_filter_refuses_retired_session_id() {
    let wire = br#"{"protocol_version":82,"session_id":"session-blue"}"#;
    let error = Envelope::<RemoteTriggerSubscriptionFilter>::decode_json(wire)
        .expect_err("version-82 filter must reject the retired session_id field");
    assert!(matches!(error, RemoteProtocolError::MessageDecode(_)));
    assert!(error.to_string().contains("session_id"), "{error}");
}

#[test]
fn remote_protocol_82_session_filter_refuses_nested_duplicate_fields() {
    ...
    let wire = br#"{"protocol_version":82,"originator":{"type":"host","scope":"a","scope":"b"}}"#;
    let error = Envelope::<RemoteProcessListFilter>::decode_json(wire)
        .expect_err("version-82 envelope must preserve nested duplicate-field rejection");
    assert!(matches!(error, RemoteProtocolError::MessageDecode(_)));
    assert!(
...
fn remote_process_dtos_json_round_trip() {
    assert_eq!(REMOTE_PROTOCOL_VERSION, 82, "remote DTO wire-shape pin");
    let start = RemoteProcessStartRequest {
        ...
        identity: Some(RemoteDeclaredProcessIdentity {
...
/// FIG-3094: an observed process carries its terminal failure as a typed
/// classification beside the display string, so a polling host branches on the
/// class instead of matching the prose. The wire spellings are pinned, and the
/// classification survives the core round trip in both directions.
#[test]
fn observed_process_failure_is_typed_and_round_trips_through_core() {
    for (core, wire) in [
        (
            lash_core::ObservedProcessFailure::Failed {
                class: lash_core::ToolFailureClass::Timeout,
        ...
        (
            lash_core::ObservedProcessFailure::Cancelled {
                origin: Some(lash_core::CancelOrigin::OperatorRequested),
            },
            serde_json::json!({ "kind": "cancelled", "origin": "operator_requested" }),
        ),
        (
            lash_core::ObservedProcessFailure::Cancelled { origin: None },
            serde_json::json!({ "kind": "cancelled" }),
        ),
    ] {
        let remote = RemoteObservedProcessFailure::from(core.clone());
        assert_eq!(
            serde_json::to_value(&remote).expect("serialize observed-process failure"),
            wire
        );
        let decoded: RemoteObservedProcessFailure =
            serde_json::from_value(wire).expect("decode observed-process failure");
