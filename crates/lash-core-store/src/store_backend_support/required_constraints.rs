...
pub const EXPECTED_CONSTRAINTS: &[ExpectedConstraint] = &[
    ...
    // FIG-3659 NOW-B's process park feed: the parked projection's pairing,
    // the singleton clock row, and the event vocabulary checks, in the
    // process registry's database.
    expected_constraint(
        &[SqliteConstraintDatabase::ProcessRegistry],
        rendered(
            "processes",
            "ck_processes_parked",
            "(parked_since_ms IS NULL) = (parked_reason_code IS NULL)",
        ),
        rendered(
            "lash_processes",
            "ck_processes_parked",
    ...
    expected_constraint(
        ...
        rendered(
            "process_park_clock",
            "ck_process_park_clock_singleton",
            "singleton = 1",
        ),
        rendered(
