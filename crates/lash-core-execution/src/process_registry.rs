...
mod tests {
    ...
    fn reg() -> InMemoryProcessDefinitionRegistry {
        InMemoryProcessDefinitionRegistry::default()
    }

    #[test]
    fn registration_is_cas_fenced() {
        let reg = reg();
    ...
    #[test]
    fn a_name_resolves_once_and_the_record_pins_the_reference() {
        let reg = reg();
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            reg.register_definition("op-1", scope(), "nightly-scan", reference("one"), None)
                ...
                .unwrap();
            let pinned_record =
                resolve_named_definition(&reg, &SessionId::from("session-a"), "nightly-scan")
                    .await
                    .unwrap();
    ...
    #[test]
    fn a_fenced_or_unregistered_name_refuses_resolution() {
        let reg = reg();
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let resolved =
                resolve_named_definition(&reg, &SessionId::from("session-a"), "unregistered-name")
                    .await
                    .unwrap();
    ...
    #[test]
    fn a_registry_reference_to_a_missing_artifact_refuses_with_a_typed_reason() {
        let reg = reg();
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            reg.register_definition("op-1", scope(), "nightly-scan", reference("one"), None)
