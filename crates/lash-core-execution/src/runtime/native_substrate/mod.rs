...
/// drives twice. A lost ask is healed by the reconcile sweep at boot and on
/// `drain_status`, or by the session's next ask: a drive admits whatever is
/// pending, not only the item that asked.
///
