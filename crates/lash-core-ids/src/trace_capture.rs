//! A `tracing` capture layer used by runtime tests to assert on emitted events.
//!
//! Formerly `crate::runtime::tests::trace_capture`; it lives behind the
//! `testing` feature so the relocated runtime integration binaries can use it.
...
impl CapturedEvent {
    ...
    pub fn field_kind(&self, name: &str) -> CapturedFieldKind {
        self.fields
            .get(name)
            .unwrap_or_else(|| panic!("event is missing field `{name}`: {self:?}"))
            .kind
...
impl EventCapture {
    ...
    #[expect(
        clippy::expect_used,
        reason = "test trace capture asserts fixture shape"
    )]
    pub fn exactly_one(&self, event: &str) -> CapturedEvent {
...
pub async fn capturing_with_capture<F, Fut, T>(body: F) -> (T, EventCapture)
where
