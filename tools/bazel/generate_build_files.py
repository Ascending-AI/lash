...
# action; a crate whose compiles measurably need more has a row. The numbers
# are resolved at analysis time: the generated `tools/bazel/exec_sizes.bzl`
# carries them, and each generated BUILD file names the lookup rather than
# the row, so a re-measurement -- or a concurrently merged PR's sizing rows --
# never rewrites a crate's BUILD file (FIG-driven merge hotspots; a stale
# literal under another PR's row used to fail `--check` on main). A row sizes
# every compile action of every target with that crate name in that package:
# Rustc, RustcMetadata and Rustdoc, and for a library its unit-test compile
# too.
#
# The Clippy and cargo-check aspects create their actions in an aspect
# context, to which Bazel never merges a target's `exec_properties`, so the
# same rows are emitted once more into `tools/bazel/pool_sizes.bzl`: a pool
# exec group per distinct request plus the matching exec-group-scoped
# properties on the `//tools/bazel:pool` execution platform. The aspects in
# `tools/bazel/clippy.bzl` read the target's `exec_properties` and run their
# action in the pool exec group named by them, where the platform's scoped
# properties size the action like the crate's row does for rule-owned
# actions.
ACTION_SIZES_PATH = ROOT / "tools/bazel/action-sizes.json"
# Must match `build --remote_default_exec_properties=...` in `.bazelrc` and
# `DEFAULT_MEMORY_KB` in `tools/bazel/action_sizes_from_log.py`.
...
load(
    "//tools/bazel:exec_sizes.bzl",
    "sized_exec_properties",
    "test_batch_budget",
)
...
# The resolved run request and batch reservation of every emitted label,
# rendered into `tools/bazel/exec_sizes.bzl`: generated BUILD files name the
# lookup, not the row, so re-measured numbers never rewrite a crate's file.
RESOLVED_TEST_RUNS: dict[str, dict[str, int]] = {}
RESOLVED_BATCH_BUDGETS: dict[str, dict[str, int]] = {}
...
def pool_exec_group_name(request: dict[str, int]) -> str:
    """The pool exec group carrying one measured request.

    The Clippy/check aspects run their action in the group named by the
    target's `exec_properties`, so the name derives from the two keys those
    properties always carry: `pool_<cpu_count>_<memory_kb>`.
    """
    return f"pool_{request['cpu_count']}_{request['memory_kb']}"


def pool_sizes_bzl() -> str:
    """The exec groups and platform properties that size aspect actions.

    `POOL_EXEC_GROUPS` is what the aspects declare; `POOL_EXEC_PROPERTIES` is
    the `//tools/bazel:pool` platform's `exec_properties`: the repository
    default request unscoped, plus each distinct measured request scoped to
    its exec group. A crate whose Clippy action runs in `pool_2_3670016`
    requests 2 cores and 3.5 GiB exactly as its Rustc action does.
    """
    requests = sorted(
        {(row["cpu_count"], row["memory_kb"]) for row in ACTION_SIZES.values()}
    )
    lines = [
        GENERATED_HEADER,
        ...
        "POOL_EXEC_GROUPS = {\n",
        "".join(
            f'    {quote(pool_exec_group_name({"cpu_count": cpu, "memory_kb": memory}))}: exec_group(),\n'
            for cpu, memory in requests
        ),
        "}\n\n",
        "POOL_EXEC_PROPERTIES = {\n",
        f'    "cpu_count": "{DEFAULT_CPU_COUNT}",\n',
        f'    "memory_kb": "{DEFAULT_MEMORY_KB}",\n',
        "".join(
            f'    {quote(f"{pool_exec_group_name({"cpu_count": cpu, "memory_kb": memory})}.{key}")}: "{value}",\n'
            for cpu, memory in requests
            for key, value in (("cpu_count", cpu), ("memory_kb", memory))
        ),
        "}\n",
    ]
    return "".join(lines)
...
def exec_sizes_bzl() -> str:
    """The tables whose rows generated BUILD files resolve by name.

    Every generated target's `exec_properties` is `sized_exec_properties(
    ...
    lines = [
        ...
        "".join(
            f"    {quote(key)}: {{"
            f'"cpu_count": {compile_request(*key.rsplit("/", 1))["cpu_count"]}, '
            f'"memory_kb": {compile_request(*key.rsplit("/", 1))["memory_kb"]}'
            "},\n"
            for key in sorted(ACTION_SIZES)
        ),
        "}\n\n",
        "TEST_RUN_REQUESTS = {\n",
        "".join(
            f"    {quote(label)}: {{"
            f'"cpu_count": {RESOLVED_TEST_RUNS[label]["cpu_count"]}, '
            f'"memory_kb": {RESOLVED_TEST_RUNS[label]["memory_kb"]}'
            "},\n"
            for label in sorted(RESOLVED_TEST_RUNS)
        ),
        "}\n\n",
        "BATCH_BUDGETS = {\n",
        "".join(
            f"    {quote(label)}: {{"
            f'"cpu_count": {RESOLVED_BATCH_BUDGETS[label]["cpu_count"]}, '
            f'"memory_kb": {RESOLVED_BATCH_BUDGETS[label]["memory_kb"]}'
            "},\n"
            for label in sorted(RESOLVED_BATCH_BUDGETS)
        ),
        "}\n\n",
...
def exec_properties_argument(
    ...
    """The `exec_properties = sized_exec_properties(...)` line.

    The row lives in generated `tools/bazel/exec_sizes.bzl`; the target names
    the lookup, never the numbers, so a sizing-table change -- its own or a
    concurrently merged PR's -- rewrites no crate's BUILD file.
    """
    exec_properties(package_name, crate_name, kind, label)
    arguments = f"{quote(package_name)}, {quote(crate_name)}"
    if kind == "test":
        arguments += f", {quote(label)}"
    return f"{' ' * indent}exec_properties = sized_exec_properties({arguments}),\n"
...
def render_package(package: dict, features: list[str]) -> tuple[str, dict]:
    ...
    if library:
        ...
        # library reproduces the workspace doc build. The crate's compile row
        # applies to its Rustdoc action as it does to Rustc: rustdoc runs the
        # same front end over the same sources.
        chunks.append(
            "rust_doc(\n"
    ...
    if len(batch_members) >= 2:
        ...
        RESOLVED_BATCH_BUDGETS[batch_label] = batch_budget(
        ...
        chunks.append(
            ...
            "lash_batch_test(\n"
            '    name = "test_batch",\n'
            f"    budget = test_batch_budget({quote(batch_label)}),\n"
            f"    jobs = {BATCH_JOBS},\n"
...
def generated(metadata: dict) -> tuple[dict[pathlib.Path, str], list[dict]]:
    ...
    # The feature-lane render above resolves its variants' run requests too;
    # emit the table only now, once every label's row is recorded.
    outputs[ROOT / "tools/bazel/exec_sizes.bzl"] = exec_sizes_bzl()
